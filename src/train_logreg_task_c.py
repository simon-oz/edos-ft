...
#!/usr/bin/env python3
# src/train_logreg_task_c.py
"""Train a TF-IDF + Logistic Regression classifier for EDOS Task C (fine-grained vector
classification). Linear models are the strongest *lexical* baseline on sparse TF-IDF text,
so this also serves as the lexical-ceiling probe for Task C.

Changes from the Task B XGBoost script:
- Estimator swapped to LogisticRegression with class_weight="balanced".
...
- Number of classes detected dynamically; confusion matrix forced square.

Usage:
  python src/train_logreg_task_c.py
  python src/train_logreg_task_c.py --C 4.0 --ngram_max 3 --stop_words none
"""
...
def fit_logreg(texts, labels, *, max_features=50000, ngram_range=(1, 2),
               stop_words="english", C=4.0, max_iter=20000):
...
def main():
    parser = argparse.ArgumentParser(description="Train TF-IDF + Logistic Regression for EDOS Task C")
    ...
    parser.add_argument("--out_dir", default="models/logreg_task_c")
    ...
    log_file = LOG_DIR / f"logreg_task_c_train_{timestamp}.log"
    ...
    t0 = time.time()
    pipeline = fit_logreg(train_df["text"].tolist(), train_df["label"].values,
    ...
    model_path = out_dir / "logreg_task_c_pipeline.joblib"
