#!/usr/bin/env python3
# src/train_xgboost_task_b.py
"""
Train a TF-IDF + XGBoost classifier for EDOS Task B (4-category classification).

Uses:
- TfidfVectorizer for n-gram features.
- XGBClassifier with balanced sample weights to handle class imbalance.

python src/train_xgboost_task_b.py
python src/train_xgboost_task_b.py --max_depth 8 --learning_rate 0.05 --n_estimators 500
"""
...
def fit_xgboost(
    ...
    *,
    ...
    # Pass sample_weights specifically to the XGBoost classifier step
    pipeline.fit(texts, labels, clf__sample_weight=sample_weights)
    return pipeline


def main():
    parser = argparse.ArgumentParser(description="Train TF-IDF + XGBoost for EDOS Task B")
    ...
    parser.add_argument("--out_dir", default="models/xgboost_task_b")
    parser.add_argument("--max_features", type=int, default=50000)
    parser.add_argument("--ngram_min", type=int, default=1)
    parser.add_argument("--ngram_max", type=int, default=2)
    parser.add_argument("--stop_words", default="english", help="stop words for TfidfVectorizer or 'none'")
    parser.add_argument("--n_estimators", type=int, default=300, help="Number of boosting rounds (trees)")
    ...
    log_file = LOG_DIR / f"xgboost_task_b_train_{timestamp}.log"
    ...
    model_path = out_dir / "xgboost_task_b_pipeline.joblib"
