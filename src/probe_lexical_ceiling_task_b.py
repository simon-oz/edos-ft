#!/usr/bin/env python3
"""probe_lexical_ceiling_task_b.py — measure the TF-IDF ceiling on EDOS Task B."""
...
def main():
    ...
    ap.add_argument("--train", default="data/processed/task_b_train.csv")
    ap.add_argument("--dev",   default="data/processed/task_b_dev.csv")
    ap.add_argument("--test",  default="data/processed/task_b_test.csv")
    a = ap.parse_args()
    tr, dv, te = (pd.read_csv(p) for p in (a.train, a.dev, a.test))
    ytr, ydv, yte = tr["label"].values, dv["label"].values, te["label"].values

    word = TfidfVectorizer(max_features=100000, ngram_range=(1, 2), sublinear_tf=True, strip_accents="unicode")
    char = TfidfVectorizer(analyzer="char_wb", ngram_range=(3, 5), max_features=200000, sublinear_tf=True)
